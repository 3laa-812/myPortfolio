...
export default function Systems() {
  ...
  const nodes = [
    ...
    },
    {
      ...
      skills: [
        { name: "PostgreSQL", val: 73 },
        { name: "SQL Server", val: 77 },
        { name: "EF Core / ORM", val: 60 },
        { name: "Schema design", val: 70 },
        { name: "Multi-tenancy", val: 60 },
      ],
    },
    {
      name: "Mobile",
            ...
            {[
              {
                ...
                detail:
                  "The React rendering model directly informs how API responses are designed. An endpoint that returns a deeply nested object for a component that needs only three fields is a frontend performance problem disguised as a backend problem.",
              },
              {
                link: "Backend ↔ Data",
                detail:
                  "N+1 query bugs are backend bugs that come from not understanding the ORM's query generation. Designing a schema without thinking about the query patterns that will run against it is building for pain later.",
              },
              {
                link: "CS Fundamentals ↔ Everything",
                detail:
                  "The hash table isn't just a data structure — it's why JavaScript object lookup is O(1), why database indexes work, why cache keys need to be designed carefully. Fundamentals are pattern recognition across domains.",
              },
              {
                link: "Mobile ↔ API",
                detail:
                  "A mobile API is a different contract than a web API. Mobile clients are on unreliable networks. They paginate more aggressively. They need to handle partial failures gracefully. This changed how I think about API design.",
              },
            ].map((conn, idx) => (
              <div
                key={idx}
                className="flex flex-col sm:flex-row gap-1.5 sm:gap-3 items-start text-xs leading-relaxed"
              >
                <span className="font-mono font-semibold text-[var(--accent2)] shrink-0">
                  {conn.link}
                </span>
                <span className="text-[var(--text2)] font-sans">
                  {conn.detail}
                </span>
              </div>
            ))}
        ...
        <div className="bg-[var(--bg2)] border border-[var(--border)] rounded-lg p-5 font-mono text-xs md:text-sm leading-relaxed text-[var(--text2)] flex flex-col gap-3">
          ...
          <div className="space-y-1.5 bg-transparent border-0">
            <div>
              ...
              Frontend architecture, React ecosystem, REST design, relational DB
              modeling.
            </div>
            <div>
              ...
              Cloud infrastructure, distributed systems, performance at scale.
            </div>
            <div>
              <span className="text-[var(--accent3)] font-semibold">
                Concept-level:
              </span>{" "}
              Message queues, microservices, container orchestration.
            </div>
            <div>
              <span className="text-[var(--text3)] font-semibold">
                Not touched:
              </span>{" "}
              ML pipelines, low-level systems, real-time distributed data.
            </div>
          </div>
        </div>
      </div>
